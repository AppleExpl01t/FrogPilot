...
class SpeedLimitVisionDaemon:
  ...
  def publish(self, now, force=False):
    if force or now - self.last_publish_at >= 1.0:
      self.params_memory.put(VISION_SPEED_LIMIT_PARAM, json.dumps(self.confirmation.snapshot(now)))
  ...
  def step(self, now):
    ...
    road = self.params_memory.get("RoadName", encoding="utf-8") or ""
...
def main():
  ...
  sm = messaging.SubMaster(["deviceState", "frogpilotCarState"], frequency=RUNTIME_LOOP_HZ)
  SpeedLimitVisionDaemon(Params("/dev/shm/params"), sm, VisionIpcClient, VisionStreamType, cloudlog).run()
