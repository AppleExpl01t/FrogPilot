...
class MemoryParams:
  def __init__(self):
    self.values = {"RoadName": "Main Street"}
...
class TestRuntime:
  ...
  def test_end_to_end_frames_publish_meters_per_second(self):
    self.confirm()
    assert json.loads(self.params.get(VISION_SPEED_LIMIT_PARAM))['speedLimit'] == pytest.approx(55 * CV.MPH_TO_MS)
  ...
  def test_worker_stopping_after_camera_stall_does_not_extend_deadline(self):
    ...
    snapshot = json.loads(self.params.get(VISION_SPEED_LIMIT_PARAM))
    assert read_vision_speed_limit(snapshot, 10.2 + HEARTBEAT_TIMEOUT + 0.1) == 0

  def test_road_change_requires_new_confirmation(self):
    self.confirm()
    self.params.put('RoadName', 'Side Street')
    self.step(11)
    assert json.loads(self.params.get(VISION_SPEED_LIMIT_PARAM))['speedLimit'] == 0
